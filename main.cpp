...
#include <thread>
#include "src/WaterLogics.h"
#include "src/GameUtils.h"
#include "src/CoinLogics.h"
...
float h = yf - yi;
float tw, th, tw2, th2;
int tileSetCols = 4, tileSetRows = 1;
float tileW, tileW2;
float tileH, tileH2;
...
int main()
{
	...
	tmap = readMap("C:\\Users\\eduar\\source\\repos\\graphics-processing-final-project\\terrain1.tmap");
	...
	GLuint tid;
	loadTexture(tid, "C:\\Users\\eduar\\source\\repos\\graphics-processing-final-project\\terrain.png");
	...
	parse_file_into_str("C:\\Users\\eduar\\source\\repos\\graphics-processing-final-project\\_geral_vs.glsl", vertex_shader, 1024 * 256);
	parse_file_into_str("C:\\Users\\eduar\\source\\repos\\graphics-processing-final-project\\_geral_fs.glsl", fragment_shader, 1024 * 256);
	...
	/*
	 * Start a new thread to spread the water while the
	 * main thread is rendering the terrain.
	 **/
	thread([=]()
				 { startSpreadingWater(tmap); })
			.detach();
	thread([=]()
				 { startSpawningCoins(tmap); })
			.detach();

	// glEnable(GL_DEPTH_TEST);
	while (!glfwWindowShouldClose(g_window))
	{
		if (hasCollectedCoin(tmap, playerX, playerY))
		{
			multiplier++;
		}
		...
		{
			int totalScore = getElapsedTimeInSeconds() * multiplier;
			cout << "\nJogo finalizado.\nVocê fez " << totalScore << " pontos!\n"
					 << endl;
			if (totalScore >= 60)
			{
				cout << "Parabéns, você venceu! Sua pontuação foi maior que 60.\n"
						 << endl;
			}
			glfwSetWindowShouldClose(g_window, 1);
		}

		tmap->setTile(playerX, playerY, 2);
		...
		for (int r = 0; r < tmap->getHeight(); r++)
		{
			for (int c = 0; c < tmap->getWidth(); c++)
			{
				...
				glUniform1f(glGetUniformLocation(shader_programme, "weight"), (c == playerX) && (r == playerY) ? 0.1 : 0.0);
